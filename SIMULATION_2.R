###############################################################
###    This file reproduces simulation results in Table 3   ###
###############################################################
require("nlme")
...
sim_func<-function(eff=0.2,t1=2,t2=2,n1=10,n2=10,rho1=0.05,alpha1=0.03,rho2=0.05,alpha2=0.02,sigma1=1,nsim=1000,seed=1112){
  ...
  for (s in 1:nsim){
    ...
    while (exit==FALSE){
      ...
      fit0 = try(lme(y0 ~ trt, random = list(sid = ~ 1, tid = ~trt-1),weights = varIdent(form= ~ 1 | trt),                  #fit0: model fitting using outcome generated under null
                     data=df),silent=T)
      fit1 = try(lme(y ~ trt, random = list(sid = ~ 1, tid = ~trt-1),weights = varIdent(form= ~ 1 | trt),                   #fit1: model fitting using outcome generated under alternative
                     data=df),silent=T)
...
#simulation set 2 ICCs configuration
rho1 <- c(0.02,0.04,0.08,0.20)         #rho1: within-therapist ICC for treatment arm
T1 <- c(2,4,6,10)                      #T1: number of therapists in treatment arm
...
for (i in 1:length(rho1)){
    for (u in 1:length(T1)){
      fidata <- rbind(fidata,sim_func(eff=0.2,t1=T1[u],t2=1,n1=60/T1[u],n2=60,
                                      rho1=rho1[i],alpha1=0.25*rho1[i],rho2=0.5*rho1[i],alpha2=0.5*rho1[i],
...
for (i in 1:length(rho1)){
  for (u in 1:length(T1)){
    fidata <- rbind(fidata,sim_func(eff=0.2,t1=T1[u],t2=1,n1=60/T1[u],n2=60,
                                    rho1=rho1[i],alpha1=0.25*rho1[i],rho2=0.75*rho1[i],alpha2=0.75*rho1[i],
                                    sigma1=1,nsim=5000,seed=920784642))
